	...
	if req.AttributeValues != nil {
		if len(*req.AttributeValues) != 0 {
			for i := 0; i < len(*req.AttributeValues); i++ {
				attrValue := *req.AttributeValues
				if attrValue[i].ID != 0 {
					v.Add(fmt.Sprintf("attribute_values[%d][id]", i), strconv.Itoa(attrValue[i].ID))
				}
				if attrValue[i].BoolValue != nil {
					v.Add(fmt.Sprintf("attribute_values[%d][bool_value]", i), strconv.FormatBool(*attrValue[i].BoolValue))
				}
				if attrValue[i].NumValue != nil {
					v.Add(fmt.Sprintf("attribute_values[%d][num_value]", i), strconv.FormatFloat(*attrValue[i].NumValue, 'g', -1, 64))
				}
				if attrValue[i].StrValue != nil {
					v.Add(fmt.Sprintf("attribute_values[%d][str_value]", i), *attrValue[i].StrValue)
				}
			}
		}
	}
